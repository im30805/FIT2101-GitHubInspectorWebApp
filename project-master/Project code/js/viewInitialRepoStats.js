let initialRepoIndex = localStorage.getItem("Selected repository" + "-selectedIndex");
let getInitialRepos = localStorage.getItem("Initial repository");
let getInitialReposObj = JSON.parse(getInitialRepos);
...
if (initialRepoIndex !== null)
{
    repoName = getInitialReposObj[initialRepoIndex];

    for(let i = 0; i < repoName.length; i++){
      if(repoName[i] === "/"){
        shortName = repoName.substr(i+1,repoName.length-1);
      }
    }
    document.getElementById("headerBarTitle").textContent =  "Statistics for " + shortName;

    let getHistoryKey = localStorage.getItem("Recent viewed repository");
    let getHistoryObj = JSON.parse(getHistoryKey);
    let historyArray = []

    if(getHistoryObj === null){
       if (typeof(Storage) !== "undefined")
       {
          historyArray.unshift(repoName);
          //chg obj to string and store
          let getRepoString= JSON.stringify(historyArray);
          localStorage.setItem("Recent viewed repository", getRepoString);
    ...
    else if(getHistoryObj !== null){
        if (typeof(Storage) !== "undefined")
        {
           historyArray = getHistoryObj;
           historyArray.unshift(repoName);
           let noRepeatRepo = historyArray.filter(onlyUnique);

           let getRepoString= JSON.stringify(noRepeatRepo);
           localStorage.setItem("Recent viewed repository", getRepoString);
...
function getLineStats(result){
  ...
  }// This function calculates the avearge response time to an array
...
// This function gets the average update time to a file
...
// This function gets the bulk of the data from the github api. it has links to other functions which assist it in processing the data.
...
//This function gets the info on th authors of commits on a file. Using this we can find the total number of people who committed to a file, aswell as their
//names and the average update time between those commits
...
function getTimeBetweenUpdates(file_result){
  ...
  avge_time_per_file = Math.round(avge_time_per_file/8.64e+7);
...
//Async requires all previous function calls to be executed first. This helps mitigte the side effects of that.
function dummy(){
    return 1;
  }
//////////////
    ...
    let chart_3 = new Chart(numOfLinesContributedChart, {
      ...
      let chart_5 = new Chart(average_update_chart, {
                          ...
                          data: {
                            labels:file_names,
                            datasets:[{
                              label:'Time between updates (days)',
                              data:time_between_updates_to_files,
                              backgroundColor: "#37bf5c",
                            }]
                          },
                          options: {
                            title:{
                            ...
                            scales: {
                              ...
                              yAxes: [{
                                scaleLabel: {
                                  display: true,
                                  fontSize: 14,
                                  labelString: 'Time between updates(days)'
                                },
                                ticks: {
                                  beginAtZero: true
                                }
                              ...
                              scales: {
                                ...
                                yAxes: [{
                                  scaleLabel: {
                                    display: true,
                                    fontSize: 14,
                                    labelString: 'Total number of work was check in'
                                  },
                                  ticks: {
                                    beginAtZero: true
                                  }
